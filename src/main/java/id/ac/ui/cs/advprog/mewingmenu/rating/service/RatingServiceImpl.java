...
public class RatingServiceImpl implements RatingService {
    ...
    public Rating addRating(Rating rating) {
        Optional<Rating> existingRating = ratingRepository.findByUserIdAndMenu(rating.getUserId(), rating.getMenu());
    ...
    @Override
    public Rating updateRating(String ratingId, Rating updatedRating, String userId) {
        Optional<Rating> ratingOpt = ratingRepository.findById(ratingId);
        if (ratingOpt.isEmpty()) {
            throw new IllegalStateException("Rating not found");
        }
